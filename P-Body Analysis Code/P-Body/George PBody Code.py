...
def image_normalization(image, alpha):
    """Normalizes images in similiar way to ImageJ, moves max and min to highest and lowest bar on histogram
    with more than total_pixels/alpha excluding bins with more than 10% of pixels"""
...
def process_condition(
    ...
    for matched in matched_files:
        ...
        # Correction intended to avoid P-Bodies detected around the nucleus due to dim nuclear signal
        ...
        # Soft thresholding done to fill in donut P-Body shapes (rarely needed)
        ...
        # Difference of Gaussian used to detect differences in inetnsity compared to background, sign of P-Body
        dog_image = difference_of_gaussians(adjusted_image, low_sigma=low_sigma, high_sigma=high_sigma)
        dog_squared = dog_image ** 2

        # Find max positive derivitive of DoG and scale back to set DoG trheshold
        ...
        # Combine masks from Soft Thresholding and DoG Thresholding
        ...
        # Filter mask to get P-Bodies that match requirements
        ...
        # Creates PB mask and gets number of P-bodies per condition and per cell
        ...
        # Visualizations aid with quality control
...
# Change base foider to folder that contains sub folders by condition with the raw images and nuclear and cell masks
...
# Change parameters depending on experiment, they are explained in the supporting documentation
process_conditions(
